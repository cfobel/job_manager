...
def get_sharcnet_jobs(connection, queue):
    command = 'PATH=%s \nsqjobs -l%s' % (SharcNetTrial.PATH, queue if queue != 'a' else '')
    ssh_stdin, ssh_stdout, ssh_stderr = connection.exec_command(command)
    ssh_stdout.channel.recv_exit_status()
    return parse_sharcnet_jobs(ssh_stdout.read())
...
if __name__ == "__main__":
    ...
    x = get_sharcnet_jobs(S, sys.argv[1][0])
