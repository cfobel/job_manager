...
def count(trial_file):
    ...
    ct = len([k for k, v in t.items() if k not in ['default_script', 'default_results']])
    t.close()
    return ct
...
def run_parameters(trial_file, sharc_filter=lambda x: False,
    ...
    t = shelve.open(trial_file, writeback=True)
    for p, s, id_ in sub:
        t[p][Trial.ID] = id_
        t[p][Trial.STATE] = Trial.SUBMITTED
        t[p][Trial.QUEUE] = s

    t['default_script'] = prog_path
    t['default_results'] = result_path
    t.close()
